...
> **주의**: Apple Silicon (M1/M2/M3) Mac을 사용하는 경우 아래 bde2020 이미지를 사용하세요. Intel Mac이나 Linux에서는 apache/hadoop 이미지도 사용 가능합니다. ([플랫폼별 주의사항](#플랫폼별-주의사항) 참고)
...
<details>
<summary><strong>Intel Mac / Linux용 대안: apache/hadoop 이미지</strong></summary>

Intel 기반 Mac이나 Linux를 사용하는 경우, apache/hadoop 공식 이미지도 사용 가능합니다:

```yaml
services:
  namenode:
    image: apache/hadoop:3.3.6
    platform: linux/amd64  # Intel Mac의 경우 명시
    container_name: hadoop-namenode
    hostname: namenode
  ...
  # datanode1, datanode2, datanode3 설정은 위와 유사
  # (각 서비스에 platform: linux/amd64 추가)
```
...
> **참고**: bde2020 이미지를 사용하는 경우, NameNode가 자동으로 포맷됩니다. 별도의 포맷 작업이 필요 없습니다.

apache/hadoop 이미지를 사용하는 경우에만 아래 명령어로 수동 포맷이 필요합니다:
...
**권장 이미지**: `bde2020/hadoop-namenode` 및 `bde2020/hadoop-datanode`

**이유**:
- apache/hadoop 공식 이미지는 ARM64 아키텍처를 완전히 지원하지 않음
- Segmentation Fault 또는 성능 저하 발생 가능
- bde2020 이미지는 멀티 아키텍처를 지원하여 안정적
...
**권장 이미지**: `apache/hadoop:3.3.6` 또는 `bde2020` 모두 사용 가능

**apache/hadoop 이미지 사용 시**:
- `platform: linux/amd64` 설정 권장
- NameNode 수동 포맷 필요

## 자주 발생하는 문제 해결

### 문제 1: Apple Silicon에서 Segmentation Fault
...
**원인**: apache/hadoop 이미지가 ARM64를 완전히 지원하지 않음

**해결**:
1. bde2020 이미지로 변경 (권장)
2. 또는 Rosetta 2를 사용한 에뮬레이션 (느릴 수 있음)
